...
function shareOnSocialMedea(a, b, d, e) {
    switch (a) {
        ...
        case "google-plus":
            ...
            break;
        case "pinterest":
            window.open("http://www.pinterest.com/pin/create/button/?media=" + e + "&url=" + b, "sharerpinterest", "toolbar=0,status=0,width=660,height=445");
            break;
        case "whatsapp":
            window.open("http://api.whatsapp.com/send?text=" + b, "sharerpinterest", "toolbar=0,status=0,width=660,height=445");
            break;
        case "messenger":
            window.open("https://www.facebook.com/dialog/send?app_id=6353976667955116&link=" + encodeURIComponent(b) + "&redirect_uri=" + encodeURIComponent(location.href), "toolbar=0,status=0,width=660,height=445");
    }
}
function isMobile() {
...
$(function () {
    $(".text-danger").each(function () {
        $(this).parents(".form-group").addClass("ha-error");
    });
...
var cart = {
            ...
            $.ajax({
                ...
                success: function (b) {
                    ...
                    b.success &&
                        ...
                        d.reload(),
                        app.event.trigger("addToCart", b),
                        fbq("track", "AddToCart", { content_ids: [a], content_type: "product", value: b.item_total, currency: "BDT" }),
                        window.gtag && gtag("event", "add_to_cart", { items: [{ item_name: b.name, item_id: b.product_id, price: b.price, currency: "BDT" }] }));
                },
            });
        },
        update: function (a, b) {
            ...
            $.ajax({
                ...
                success: function (a) {
                    "checkout/cart" == getURLVar("route") || "checkout/checkout" == getURLVar("route")
                        ...
                        : ($(".mc-toggler .text").html(a.total), $(".mc-toggler .counter").text(a.count).attr("data-count", a.count), $(".mc-toggler").attr("title", a.total), d.reload());
                },
            });
        },
        remove: function (a, b) {
        ...
        _bulk_add: function (a) {
            var b = this;
            $.ajax({
                url: "checkout/cart/bulk_add",
                method: "post",
                data: a,
                dataType: "json",
                contentType: a instanceof FormData ? !1 : "application/x-www-form-urlencoded",
                beforeSend: function () {
                    $("#cart > button").button("loading");
                },
                complete: function () {
                ...
                success: function (a) {
                    var d = new Popup(a.message, "mgs-popup ");
                    d.render();
                    d.el.find("#form-cart").on("submit", function (a) {
                        a.preventDefault();
                        d.close();
                        b._bulk_add(new FormData(this));
                    });
                    a.success &&
                        (setTimeout(function () {
                            $(".mc-toggler .text").html(a.total);
    ...
    wishlist = {
        add: function (a) {
            $.ajax({
                ...
                success: function (b) {
                    $(".alert").remove();
                    b.success && showMsg(b.success);
                    b.info && showMsg(b.info, "error");
                    $("#wishlist-total span").html(b.total);
                    $("#wishlist-total").attr("title", b.total);
                    fbq("track", "AddToWishlist");
                    window.gtag && gtag("event", "add_to_wishlist", { currency: "BDT", value: b.price, items: [{ item_name: b.name, item_id: a, price: b.price, quantity: 1 }] });
                },
            });
        },
        remove: function () {},
    ...
    stock_request = {
        add: function (a) {
            $.ajax({
                ...
                method: "post",
                data: { product_id: a, referrer: location.href },
                dataType: "json",
                beforeSend: function () {},
                complete: function () {},
                success: function (a) {
