...
<div class="app-sidebar-logo px-6" id="kt_app_sidebar_logo">
    <!--begin::Logo image-->
    @if (config('app.admin-logo') != null)
        <a href="{{ url('/') }}" target="_blank">
            <img alt="Logo" src="{{ asset(config('app.admin-logo')) }}" class="h-55px app-sidebar-logo-default" />
            <img alt="Logo" src="{{ asset(config('app.admin-logo')) }}" class="h-55px app-sidebar-logo-minimize" />
        </a>
    @else
       <a href="{{ url('/') }}" target="_blank">
            <h2 class="logo-title mb-0" style="color:#F1416C;">{{config('app.name')}}</h2>
       </a>
    @endif
    <!--end::Logo image-->
